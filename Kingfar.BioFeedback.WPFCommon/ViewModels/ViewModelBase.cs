...
using Prism.Ioc;
using Kingfar.BioFeedback.WPFCommon.Controls;
using System.ComponentModel;

namespace Kingfar.BioFeedback.WPFCommon.ViewModels
{
    public class ViewModelBase : ObservableObject, INavigationAware
    {
        ...
        {
            if (!ContainerLocator.Container.IsRegistered<object>(name))
            {
                return null;
            }
        ...
        protected Task<ContentDialogResult> ShowDialogAsync(string title, object dialogContent, Func<Task<bool>> checkDataFunc)
        {
        ...
        protected Task<ContentDialogResult> ShowDialogAsync(string title, object dialogContent, Func<bool> checkDataFunc)
        {
            return new ValidationContentDialog(ContentDialogService.GetContentPresenter(), checkDataFunc)
            {
                Title = title,
        ...
        protected Task<ContentDialogResult> ShowConfirmDialogAsync(string message, string primaryText = "确认", string secondaryText = "")
        {
